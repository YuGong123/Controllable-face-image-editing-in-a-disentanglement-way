...
def find_fake_freq(im, gauss_kernel, index=None):  # 生成图和原图
    padding = (gauss_kernel.shape[-1] - 1) // 2
    low_freq = gaussian_blur(im, gauss_kernel, padding=padding)                      # 低频图是彩色的模糊图
    im_gray = im[:, 0, ...] * 0.299 + im[:, 1, ...] * 0.587 + im[:, 2, ...] * 0.114  # 原图转灰度
    im_gray = im_gray.unsqueeze_(dim=1).repeat(1, 3, 1, 1)                           # 将灰度图的单通道转成3通道
    low_gray = gaussian_blur(im_gray, gauss_kernel, padding=padding)
    return torch.cat((low_freq, im_gray - low_gray),1)                               # 高频图是灰度图，通道维度连起来，6通道
